...
function NewsFeedPanel({ selectedStock }) {
  ...
  const sentiment = getNewsSentimentSummary();
  const news = newsFilter === "stock"
    ? getNewsForStock(selectedStock)
    : getLatestNews(10);
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
      ...
      {/* Sentiment bar */}
      <div style={{
        display: "flex", gap: 12, marginBottom: 14, padding: "8px 10px",
        ...
        <div style={{ flex: 1, display: "flex", gap: 2, alignItems: "center" }}>
          <div style={{ height: 6, flex: sentiment.positive, background: C.green, borderRadius: "3px 0 0 3px" }} />
          <div style={{ height: 6, flex: sentiment.neutral, background: C.textDim }} />
          <div style={{ height: 6, flex: sentiment.negative, background: C.red, borderRadius: "0 3px 3px 0" }} />
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 420, overflowY: "auto" }}>
        ...
        {news.length === 0 && (
          <div style={{ padding: 20, textAlign: "center", color: C.textDim, fontSize: 12 }}>
            No recent news for {selectedStock}
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const [view, setView] = useState("dashboard"); // "dashboard" or "portfolio"
  const [positions, setPositions] = useState(() => {
    try {
      const saved = localStorage.getItem("psx_portfolio");
  ...
  // Persist positions
  useMemo(() => {
    try { localStorage.setItem("psx_portfolio", JSON.stringify(positions)); } catch {}
  }, [positions]);
      ...
      <div style={{ padding: "16px 20px" }}>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 14 }}>
            ...
            />
            <div>
              {/* Quick stock detail for selected from signals */}
              <DetailPanel ticker={selectedStock} stock={WATCHLIST_STOCKS[selectedStock]} />
            </div>
          </div>
        )}
